...
export default function CitizenComplaintDetails({ complaint, onClose }) {
            ...
            <div className="flex h-full flex-col bg-white shadow-2xl rounded-l-3xl overflow-hidden ring-1 ring-slate-900/5 border-l border-white/40">
              ...
              <div className="px-6 py-6 sm:px-8 border-b border-slate-100 bg-white sticky top-0 z-10 flex items-center justify-between">
                <div className="flex items-start gap-4">
                  <div className="p-2.5 bg-sky-50 rounded-xl flex-shrink-0 shadow-sm border border-sky-100/50">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-slate-500 text-sm font-medium">Ref: #{String(complaint._id).substring(0, 8)}</span>
                    </div>
                  </div>
                </div>
                <button
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {complaint.location && (
                      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:border-slate-200 transition-colors">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Recorded Location</span>
                        </div>
                    ...
                    {complaint.expectedResolutionDate && (
                      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:border-slate-200 transition-colors">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Expected Resolution</span>
                        </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden sm:col-span-2 group hover:border-slate-200 transition-colors">
                       <div className="flex items-center gap-3 mb-4">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-500">System Profiling</span>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-2 pl-[44px]">
                          ...
                          <div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Urgency</span>
                            <div className="flex items-center gap-1.5 align-middle text-slate-800 text-sm font-semibold">
                          ...
                          <div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Routing Dept</span>
                            <div className="flex items-center gap-1.5 align-middle text-slate-800 text-sm font-semibold">
                       ...
                       <div className="border-l-2 border-slate-100 ml-4 pl-6 space-y-6 relative">
                          ...
                          {complaint.timeline.map((event, index) => (
                             <div key={index} className="relative group">
                                <div className="absolute w-3 h-3 bg-sky-200 rounded-full left-[-31px] top-1.5 border-2 border-white shadow-sm ring-2 ring-transparent group-hover:bg-sky-400 transition-colors"></div>
                                <div className="text-sm font-bold text-slate-800 flex items-center gap-2">
                                  {event.status} 
                                  <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest bg-slate-50 px-2 py-0.5 rounded-full">Record</span>
                                </div>
                                <p className="text-sm text-slate-500 mt-1">{event.note || "Status progressed."}</p>
                                <span className="text-xs text-slate-400 mt-2 block font-medium flex items-center gap-1">
                                  {formatDate(event.timestamp)}
                                </span>
                             </div>
                          ))}
                          <div className="absolute w-3 h-3 bg-emerald-200 rounded-full left-[-7px] bottom-0 border-2 border-white"></div>
                       </div>
                     </div>
                  )}
