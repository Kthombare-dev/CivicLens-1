...
router.get('/complaints', authenticate, adminOnly, async (req, res) => {
    try {
        ...
        const formatted = complaints.map((c, index) => ({
            ...
            citizen: c.citizen_id ? { name: c.citizen_id.name, email: c.citizen_id.email } : null,
 ...
 * @access  Admin only
 */
router.put('/complaints/:id/status', authenticate, adminOnly, async (req, res) => {
    try {
    ...
    try {
        ...
        const [total, officers] = await Promise.all([
            ...
            User.aggregate([
                ...
                {
                    $project: {
                        name: 1, email: 1, phone: 1, ward: 1, isActive: 1,
...
router.post('/officers', authenticate, adminOnly, async (req, res) => {
    try {
        ...
        if (existing)
            return sendError(res, existing.email === email ? 'Email already registered' : 'Phone already registered', 409);
 ...
 * @access  Admin only
 */
router.patch('/officers/:id/status', authenticate, adminOnly, async (req, res) => {
    try {
        const officer = await User.findOne({ _id: req.params.id, role: 'official' });
        if (!officer) return sendError(res, 'Officer not found', 404);
 ...
 * @access  Admin only
 */
router.put('/complaints/:id/assign', authenticate, adminOnly, async (req, res) => {
    try {
        ...
        if (!complaint) return sendError(res, 'Complaint not found', 404);
        if (!officer)   return sendError(res, 'Officer not found or not an official', 404);
...
router.get('/analytics', authenticate, adminOnly, async (req, res) => {
    try {
        ...
        return sendSuccess(res, {
            categories:    categoryData.map(d => ({ name: d._id || 'Uncategorized', count: d.count })),
